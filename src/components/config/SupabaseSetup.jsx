...
export default function SupabaseSetup() {
  ...
  const handleSave = () => {
    ...
    toast({ title: 'تم الحفظ', description: 'تم تخزين بيانات Supabase' });
  };
  ...
  const handleCopySql = () => {
    if (sql) {
      ...
      toast({ title: 'تم النسخ', description: 'تم نسخ كود SQL إلى الحافظة' });
    }
  };
  ...
  const handleUpload = async () => {
    ...
    if (!supabase) {
      toast({ title: 'خطأ', description: 'Supabase غير مُعد', variant: 'destructive' });
      return;
    }
    ...
    if (rows.length) {
      ...
      if (error) {
        ...
        toast({ title: 'تم الرفع', description: 'تم رفع البيانات بنجاح' });
      }
    }
  };
    ...
    <Card className="glass-effect border-sky-500/30 max-w-xl mx-auto">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex gap-2 mt-2">
          <Button onClick={handleGenerate}>توليد كود SQL</Button>
          {sql && <Button onClick={handleCopySql}>نسخ الكود</Button>}
        </div>
        {sql && (
